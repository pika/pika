...
pika is a pure-Python implementation of the AMQP 0-9-1 protocol. It targets Python 3 with `requires-python >= 3.7`. The CI matrix tests Python 3.10 through 3.14. Older Python 3 versions (3.7 through 3.9) are not tested but should still work.
...
`pika/spec.py` is generated by `utils/codegen.py`. Never edit `spec.py` directly. Changes to spec classes or constants require modifying the code generator and regenerating the file. Pull requests that modify `spec.py` without corresponding `utils/codegen.py` changes will be rejected.
