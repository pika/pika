...
tests/typing/           # type-checker fixtures (see hatch run typecheck-fixtures)
...
- **Type checking:** [mypy](https://mypy-lang.org/). Configuration is in
  `mypy.ini`. Run `hatch run typecheck`. `mypy.ini` sets `packages = pika`, so
  the main run never covers code that consumes pika from the outside. Downstream
  type guarantees are pinned by fixtures in `tests/typing/`; run them with
  `hatch run typecheck-fixtures`. Both run in the mypy CI workflow.
