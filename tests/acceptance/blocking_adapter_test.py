...
class TestCreateAndCloseConnectionWithChannelAndConsumer(BlockingTestCaseBase):
    ...
    def test(self):
        ...
        # Declare a new queue
        ch.queue_declare(q_name, auto_delete=True, exclusive=True)
...
class TestQueueDeclareAndDelete(BlockingTestCaseBase):
    ...
    def test(self):
        ...
        # Declare a new queue
        frame = ch.queue_declare(q_name, auto_delete=True, exclusive=True)
...
class TestBasicGet(BlockingTestCaseBase):
    ...
    def test(self):
        ...
        connection = self._connect()
        LOGGER.info('%s CONNECTED (%s)', datetime.now(timezone.utc), self)

        ch = connection.channel()
        LOGGER.info('%s CREATED CHANNEL (%s)', datetime.now(timezone.utc), self)
...
class TestTwoBasicConsumersOnSameChannel(BlockingTestCaseBase):
    ...
    def test(self):  # pylint: disable=R0914
        ...
        ch.queue_declare(q1_name, auto_delete=True, exclusive=True)
        ch.queue_bind(q1_name, exchange=exg_name, routing_key=q1_routing_key)

        ch.queue_declare(q2_name, auto_delete=True, exclusive=True)
...
class TestPublishFromBasicConsumeCallback(BlockingTestCaseBase):
    ...
    def test(self):
        ...
        ch.queue_declare(src_q_name, auto_delete=True, exclusive=True)
        ch.queue_declare(dest_q_name, auto_delete=True, exclusive=True)
...
class TestStopConsumingFromBasicConsumeCallback(BlockingTestCaseBase):
    ...
    def test(self):
        ...
        # Declare the queue
        ch.queue_declare(q_name, auto_delete=False, exclusive=True)
