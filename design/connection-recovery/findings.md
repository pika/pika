...
`proposal-recovery.md` originally treated recovery as "purely additive": a call made while reconnecting "ran exactly as it would on a healthy connection," and no code path read recovery state. In practice that would mean a publish during recovery landing as either a `TimeoutError` (work scheduled onto a stopped IOLoop) or a generic "channel is closed", with no synchronous way to ask "is this connection recovering?" and nothing an app could catch reactively.
