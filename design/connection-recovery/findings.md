...
Treating recovery as "purely additive" - a call made while reconnecting running "exactly as it would on a healthy connection", with no code path reading recovery state - is the design these measurements rule out. In pika it would mean a publish during recovery landing as either a `TimeoutError` (work scheduled onto a stopped IOLoop) or a generic "channel is closed", with no synchronous way to ask "is this connection recovering?" and nothing an application could catch reactively.

The measured behavior of both reference clients argues the other way: they fail fast, and amqp091-go, whose docs that framing cited for "runs as on a healthy connection", actually requires the application to gate on connection state and can cause a protocol violation if it does not. These measurements are why the proposal was rewritten around the opposite contract: surface recovery as a first-class, observable state and fail in-flight operations with a dedicated, catchable exception, rather than presenting recovery as transparent. `design-state-machine.md` records how that framing was derived.
