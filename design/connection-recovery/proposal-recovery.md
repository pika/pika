...
| 8 | `Channel.confirm_delivery`, `add_on_return_callback`, `add_on_cancel_callback`, `Connection.add_on_connection_blocked_callback` / `_unblocked_callback` | Callbacks registered on the raw channel or raw connection, destroyed by `callbacks.cleanup` on channel close or by the whole `CallbackManager` going away with the inner connection | Re-register. See "Re-registration is wider than user channel callbacks" |
...
So the transition to `RECOVERING` drains the loop's pending callbacks belonging to the superseded generation, rather than letting them run against the new one. Draining rather than executing is right because every one of them was admitted against a connection that no longer exists; a queued publish is exactly as stale as a queued ack.
...
Scoping the rule to user channel callbacks leaves five registrations unrestored.
...
- **`_confirm_select_ok` is reset under `_confirm_lock`, not `_channel_waiters_lock`.** `confirm_delivery` short-circuits on that field under its own per-channel lock, which appears nowhere else in this document. The reopen path takes the same lock when clearing it, or the short-circuit races the re-arm.
...
- **`_register_connection_event_callback` inlines its own `_closed_reason` test** rather than calling `_check_not_closed`, so it is missed by a guard list derived from callers of that method. It needs the state test too.
...
- **Phases 2 and 3 must not ship a default `RecoveryConfig()` that reports success while restoring nothing.** They behave as `topology_recovery_mode=DISABLED`, so the default has to be that mode until phase 4 lands, or `on_recovery_succeeded` fires for a connection whose consumers are all gone.
...
Every state change goes through one method, and it owes four things: the terminal rule (once `CLOSING`/`CLOSED`, only `CLOSED` may follow), waking blocked waiters, dispatching ordered listeners, and stashing the per-pass payload the recovery callbacks carry. It was previously named once, in a code comment, and specified nowhere - which is how a method owning the terminal rule ended up absent from the file-by-file change list.
...
So `_transition` takes the lock only for the compare-and-set and the waiter wake, releases it, and *then* dispatches listeners. Listener reentrancy is therefore safe by construction rather than by asking listeners to behave. It runs on the loop thread only; a caller thread wanting a transition schedules one.
