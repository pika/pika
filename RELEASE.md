...
* Create `pika-1.4.4` branch
* Update `CHANGELOG.md`. Be sure to use the `--since-tag X.Y.Z` argument:
    ```
    github_changelog_generator --user pika --project pika --token $TOKEN --since-tag 1.4.4 --future-release 1.4.4 --release-branch 1.4.x
    ```
...
* Commit changes to `pika-1.4.4` branch and push:
    ```
    git commit -a -m 'pika 1.4.4' && git push origin pika-1.4.4
    ```
...
* Tag version (using your GPG keyid) and push:
    ```
    git tag -a -s -u B1B82CC0CF84BA70147EBD05D99DE30E43EAE440 -m 'pika 1.4.4' '1.4.4' && git push --tags
    ```
...
* Note the release's milestone number, then create release via GitHub Release UI or `gh` command like:
    ...
    gh release create '1.4.4' --notes 'https://pypi.org/project/pika/1.4.4/ | [GitHub milestone](https://github.com/pika/pika/milestone/27?closed=1)' ./dist/*
    ```
* Ensure the publish build succeeded. Example success output looks like this:
    ```
    Checking dist/pika-1.4.4-py3-none-any.whl: PASSED
    Checking dist/pika-1.4.4.tar.gz: PASSED
    Uploading distributions to https://upload.pypi.org/legacy/
    Uploading pika-1.4.4-py3-none-any.whl
    ...
    ...
    ...
    View at:
    https://pypi.org/project/pika/1.4.4/
    ```

* View the release on PyPI: https://pypi.org/project/pika/1.4.4/
