...
Applications primarily interface with RabbitMQ via the :py:meth:`pika.channel.Channel` object. It is
with the :py:meth:`pika.channel.Channel` that you issue commands to RabbitMQ to send messages and
receive messages.

:py:meth:`pika.channel.Channel` is invoked by the active connection by calling *connection.channel(on_connected_callback)*. While you may
...
Example that sends 1 message::
    ...
    # Bind the queue
    channel.queue_bind('test', 'test_exchange', 'test')
...
command specification. The :py:meth:`pika.channel.Channel` documentation below
